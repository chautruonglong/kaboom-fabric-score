...
func TestMain(m *testing.M) {
	...
	peerDirPrefix := filepath.Join(testdir, "crypto-config", "peer-organizations")
	testPeers = testPeerSet{
...
func createUserSigner(t *testing.T) *signer {
	identityDir := filepath.Join(testdir, "crypto-config", "peer-organizations", "org1.example.com", "users", "User1@org1.example.com", "msp")
...
func createAdminSigner(t *testing.T) *signer {
	identityDir := filepath.Join(testdir, "crypto-config", "peer-organizations", "org1.example.com", "users", "Admin@org1.example.com", "msp")
...
func createMSPManager(t *testing.T, dir string, configs map[string]*msprotos.FabricMSPConfig) msp.MSPManager {
	...
	for _, mspConf := range []struct {
		...
		{
			mspID:  "Org1MSP",
			mspDir: filepath.Join(dir, "crypto-config", "peer-organizations", "org1.example.com", "msp"),
		},
		{
			mspID:  "Org2MSP",
			mspDir: filepath.Join(dir, "crypto-config", "peer-organizations", "org2.example.com", "msp"),
		},
		{
			mspID:  "OrdererMSP",
			mspDir: filepath.Join(dir, "crypto-config", "orderer-organizations", "example.com", "msp"),
		},
	} {
...
func createChannelConfig(t *testing.T, cryptoConfigDir string) *common.Config {
	...
	for _, org := range channelConfig.Orderer.Organizations {
		org.MSPDir = filepath.Join(cryptoConfigDir, "orderer-organizations", "example.com", "msp")
	}
	for i, org := range channelConfig.Application.Organizations {
		...
		org.MSPDir = filepath.Join(cryptoConfigDir, "peer-organizations", fmt.Sprintf("org%d.example.com", i+1), "msp")
	}
