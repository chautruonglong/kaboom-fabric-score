...
var _ = Describe("EndToEnd Crash Fault Tolerance", func() {
	...
	When("orderer TLS certificates expire", func() {
		It("is still possible to recover", func() {
			...
			ordererTLSCAKeyPath := filepath.Join(network.RootDir, "crypto", "orderer-organizations",
				ordererDomain, "tlsca", "priv_sk")

			ordererTLSCAKey, err := ioutil.ReadFile(ordererTLSCAKeyPath)
			Expect(err).NotTo(HaveOccurred())

			ordererTLSCACertPath := filepath.Join(network.RootDir, "crypto", "orderer-organizations",
	...
	When("admin certificate expires", func() {
		It("is still possible to replace them", func() {
			...
			ordererCAKeyPath := filepath.Join(network.RootDir, "crypto", "orderer-organizations", ordererDomain, "ca", "priv_sk")
			...
			adminPath := filepath.Join(network.RootDir, "crypto", "orderer-organizations",
