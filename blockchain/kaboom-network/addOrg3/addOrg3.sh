...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../.dist/organizations/peer-organizations/org3.example.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../.dist/organizations/peer-organizations/org3.example.com/peers/peer0.org3.example.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d ../.dist/organizations/orderer-organizations ]; then
    fatalln "ERROR: Please, run ./network.sh up createChannel first."
  fi

  # generate artifacts if they don't exist
  if [ ! -d "../.dist/organizations/peer-organizations/org3.example.com" ]; then
    generateOrg3
