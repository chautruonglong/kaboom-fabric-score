...
packageChaincode() {
  mkdir -p ${PWD}/.dist/chaincode
  
  set -x
  peer lifecycle chaincode package ${PWD}/.dist/chaincode/${CC_NAME}.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ${CC_NAME}_${CC_VERSION} >&${PWD}/.dist/logs/log.txt
  res=$?
  { set +x; } 2>/dev/null
